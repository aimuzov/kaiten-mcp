...
async function main() {
  ...
  // 1) register
  const reg = await (await fetch(`${BASE}/register`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ redirect_uris: [redirectUri] }) })).json() as { client_id?: string };
  if (!reg.client_id) throw new Error("register: нет client_id");

  // 2) authorize POST (имитируем сабмит формы)
  const authBody = new URLSearchParams({ kaiten_token: "dummy-token", kaiten_url: "https://example.kaiten.ru", redirect_uri: redirectUri, code_challenge: challenge, state: "s1" });
  const authRes = await fetch(`${BASE}/authorize`, { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: authBody, redirect: "manual" });
  ...
  // 3) token
  const tokenBody = new URLSearchParams({ grant_type: "authorization_code", code, code_verifier: verifier, redirect_uri: redirectUri });
  const tokRes = await fetch(`${BASE}/token`, { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: tokenBody });
  ...
  process.stdout.write(`OK: flow пройден. Инструментов: ${tools.length}. Вызов инструмента вернулся (isError=${(res as { isError?: boolean }).isError ?? false}).\n`);
}

main().catch((e) => { process.stderr.write(`flow-test упал: ${e?.stack ?? e}\n`); process.exit(1); });
