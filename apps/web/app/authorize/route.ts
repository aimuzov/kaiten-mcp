...
export async function GET(req: Request) {
	...
	if (!codeChallenge || p.get('code_challenge_method') !== 'S256')
		return badRequest('нужен code_challenge с методом S256')
...
export async function POST(req: Request) {
	...
	const kaitenUrl =
		(getConfiguredKaitenUrl() ?? String(form.get('kaiten_url') ?? '').trim()) || undefined

	if (!kaitenToken) return new NextResponse('Не указан Kaiten токен', { status: 400 })
	if (!redirectUri || !codeChallenge)
		return new NextResponse('Битый запрос авторизации', { status: 400 })
	if (!isAllowedRedirectUri(redirectUri))
		return new NextResponse('redirect_uri не разрешён', { status: 400 })
