...
export async function POST(req: Request) {
  ...
  if (grantType === "authorization_code") {
    ...
    let payload;
    try { payload = await openToken(code, secret); } catch { return err("invalid_grant"); }
    ...
    if (!payload.code_challenge || !verifyPkceS256(verifier, payload.code_challenge)) return err("invalid_grant");
    ...
    return NextResponse.json({ access_token, refresh_token, token_type: "Bearer", expires_in: 12 * 3600 });
  }

  if (grantType === "refresh_token") {
    const refresh = String(form.get("refresh_token") ?? "");
    let payload;
    try { payload = await openToken(refresh, secret); } catch { return err("invalid_grant"); }
