...
import { loadConfig, CredentialsProvider, KaitenClient, Logger, makeToolContext, registerAllTools } from "@kaiten-mcp/core";
  ...
  credentials.setElicitor(async (need) => {
    ...
    if (need.token) {
      properties.api_token = {
        ...
        description: "Персональный API-токен (профиль → API-ключи). Хранится только в памяти на время сессии.",
        minLength: 1,
      };
