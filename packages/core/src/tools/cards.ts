...
export function registerCardTools(ctx: ToolContext): void {
	...
	server.registerTool(
		'kaiten_get_card',
		{
			title: 'Карточка',
			description:
				'Возвращает карточку по ID со всеми полями (описание, участники, теги, сроки и т.д.).',
			inputSchema: {
	...
	server.registerTool(
		'kaiten_update_card',
		{
			...
			description: 'Обновляет поля карточки. Передавайте только те поля, которые нужно изменить.',
			inputSchema: {
				card_id: z.number().int().describe('ID карточки'),
	...
	server.registerTool(
		...
		(args, extra) =>
			run(ctx, extra, (client) => client.patch(`/cards/${args.card_id}`, { condition: 2 })),
	)
}
